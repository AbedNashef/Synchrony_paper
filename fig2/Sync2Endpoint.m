...
for j=1:length(dates)
    ...
    for i=1:size(FR,1)-1
        for ii=i+1:size(FR,1)
            ...
            if overlap==0
                ...
                sumSI= nansum((cell1(:,tix).*cell2(:,tix))./repmat(Pr_S1.*Pr_S2,size(all_gain,1),1),2); % get SI for each trial individually
    ...
    clear FR FR2gain this_S nX nY nZ endpoint_XY
end
%%
...
scatter(mean(nanmean(XENDPOINT_HIGH_SI,2)),mean(nanmean(YENDPOINT_HIGH_SI,2)),70,'o','MarkerFaceColor','r','MarkerFaceAlpha',1,'MarkerEdgeColor','none')
scatter(mean(nanmean(XENDPOINT_LOW_SI,2)),mean(nanmean(YENDPOINT_LOW_SI,2)),70,'o','MarkerFaceColor','b','MarkerFaceAlpha',1,'MarkerEdgeColor','none')
a=axis;
plot([0 0],a(3:4),'--k')
plot(a(1:2),[0 0],'--k')
hold off
...
subplot(2,3,1)
ix= find(gain(:,1)<-.05 & gain(:,2)<-.05);
xlow=squeeze(endpoint_high2lowSI(1,ix,1));
xhigh=squeeze(endpoint_high2lowSI(1,ix,2));
ylow=squeeze(endpoint_high2lowSI(2,ix,1));
yhigh=squeeze(endpoint_high2lowSI(2,ix,2));
scatter(xlow,ylow,30,'o','MarkerFaceColor',[.3 .7 1],'MarkerEdgeColor','w','MarkerFaceAlpha',.5)
hold on
scatter(xhigh,yhigh,30,'o','MarkerFaceColor',[1 .7 .3],'MarkerEdgeColor','w','MarkerFaceAlpha',.5)
a=axis;
scatter(nanmean(xlow),nanmean(ylow),70,'sb','fill')
scatter(nanmean(xhigh),nanmean(yhigh),70,'sr','fill')
plot([0 0],a(3:4),'--k')
plot(a(1:2),[0 0],'--k')
hold off
set(gca,'TickDir','out'); box off
xlabel('\DeltaX');
ylabel('\DeltaY');
[~,p]= ttest([xlow(:); ylow(:)],[xhigh(:); yhigh(:)]);
title(p)


subplot(2,3,4)
for i=1:size(endpoint_high2lowSI,1)
    plot([i-.1 i+.1],[squeeze(endpoint_high2lowSI(i,ix,1))' squeeze(endpoint_high2lowSI(i,ix,2))'],'Color',[.5 .5 .5],'LineWidth',.5)
    hold on
    plot([i-.1 i+.1],nanmean([squeeze(endpoint_high2lowSI(i,ix,1))' squeeze(endpoint_high2lowSI(i,ix,2))'],1),'r','LineWidth',2)
    [~,p]= ttest(squeeze(endpoint_high2lowSI(i,ix,1)),squeeze(endpoint_high2lowSI(i,ix,2)));
    a=axis;
    if p<.05
        scatter(i,a(4),50,'*')
    end
...
plot([.9 3.1],[0 0],'--k')
hold off
set(gca,'XTick',1:3,'XTickLabel',{'Out' 'Up' 'Euc'},'TickDir','out'); box off
