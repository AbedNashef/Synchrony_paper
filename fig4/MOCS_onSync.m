            ...
            if ~vExclude & ~vStim
                ...
                for cc=1:length(cellData)
                    if ~isempty(cellData(cc).CS_Bin1)
                        ...
                        index= find(this_trc(:,1)>=mo_time-0.3 & this_trc(:,1)<=mo_time);
...
avgs(:,3)= nanmean(all_sync_CSoff(:,tix),2);
ix= find(all_gain(:,1)<-.05 & all_gain(:,2)<-.05);
bar([1 2 3],nanmean(avgs(ix,:),1),'FaceColor',[.9 .9 1],'EdgeColor','k')
hold on
plot([1 2 3],avgs(ix,:),'Color','b')
plot([1 2 3],nanmean(avgs(ix,:),1),'Color','k')
[~,p]= ttest(avgs(ix,1),avgs(ix,2));
if p<.05
    text(2,10,'*','FontName','Times')
end
...
ylabel('Avg. SI (0-0.3 s)')
set(gca,'TickDir','out'); box off
set(gca,'XTick',[1:3],'XTickLabel',{'CS+/+' 'CS+/-' 'CS-/-'})



subplot(2,2,4)
ix= find((all_gain(:,1)<-.05 & all_gain(:,2)<-.05));
% m=smooth(nanmean(all_sync_CSon_matched(ix,:)-all_sync_CSoff_matched(ix,:),1),sm); m=m(:)';
% s=smooth(nanstd(all_sync_CSon_matched(ix,:)-all_sync_CSoff_matched(ix,:),[],1),sm)./sqrt(length(ix)); s=s(:)';
m=smooth(nanmean(all_dsync_matched(ix,:),1),sm); m=m(:)';
s=smooth(nanstd(all_dsync_matched(ix,:),[],1),sm)./sqrt(length(ix)); s=s(:)';
xx=[ax ax(end) ax(end) fliplr(ax) ax(1) ax(1)];
yy=[m-s m(end)-s(end) m(end)+s(end) fliplr(m+s) m(1)+s(1) m(1)-s(1)];
...
m=smooth(nanmean(all_dsync_matched4shared(ix,:),1),sm); m=m(:)';
s=smooth(nanstd(all_dsync_matched4shared(ix,:),[],1),sm)./sqrt(length(ix)); s=s(:)';
xx=[ax ax(end) ax(end) fliplr(ax) ax(1) ax(1)];
yy=[m-s m(end)-s(end) m(end)+s(end) fliplr(m+s) m(1)+s(1) m(1)-s(1)];
patch(xx,yy,[0.5 1 .5],'EdgeColor','none');
plot(ax,m,'Color','g')
tmp=all_dsync_matched4shared(ix,:);
[~,p]= ttest(tmp);
scatter(ax(find(p<.05)),m(find(p<.05)),'*g')
